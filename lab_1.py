...
# A* search algorithm with debug information
def a_star_search(graph, start, goal, heuristic):
    ...
    while open_list:
        ...
        print(f"Exploring node: {current_node} with f_score: {current_f}")

        if current_node == goal:
            print(f"Goal {goal} reached with distance {g_score[goal]}")
            return g_score[goal]
        ...
        for neighbor, distance in graph[current_node].items():
            ...
            print(f"Checking neighbor: {neighbor} with tentative_g: {tentative_g}")

            if tentative_g < g_score[neighbor]:
                g_score[neighbor] = tentative_g
                f_score = tentative_g + heuristic[neighbor]
                heapq.heappush(open_list, (f_score, neighbor))
                print(f"Updated f_score for {neighbor}: {f_score}")
